...
final class AutocompleteExampleViewController: ChatViewController {
    ...
    override func configureMessageInputBar() {
        ...
        tMessage.insertText("This is a test")
        //messageInputBar.set(tMessage, animated: false)
 ...
 //       }
        //messageInputBar.sendButton.enable
        
                bSDK.registerKbdTarget(withID: tMessage, andName: "FIELD_USER", andTargetType: NORMAL_TARGET)
 //               bSDK.registerKbdTarget(withID: tPassowrd, andName: "FIELD_PWD", andTargetType: MASKED_TARGET)
                tMessage.text = nil
  //              tUsername.text = ""
    ...
    @objc
    func joinChat() {
        print("send " + (tMessage.text ?? " No text"))
        configureMessageInputBarForChat()
    }

    @objc
    func printChat() {
        print("send2 " + (tMessage.text ?? " No text"))
        tMessage.text = nil
        messageInputBar.sendButton.isEnabled = true
        //configureMessageInputBarForChat()
        let behavioSession: BehavioSession = BehavioSession(user: tMessage.text)
